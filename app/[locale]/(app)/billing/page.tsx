...
async function getBillingData(organizationId: string) {
  const [invoiceStats, paymentStats, recentInvoices] = await Promise.all([
    billingRepo.getInvoiceStats(organizationId),
    billingRepo.getPaymentStats(organizationId),
    billingRepo.listInvoices(organizationId, { status: 'open' }, { perPage: 5 })
  ])
...
export default async function BillingPage() {
  ...
  const { invoiceStats, paymentStats, recentInvoices } = await getBillingData(session.organizationId)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {recentInvoices.data.length > 0 ? '5 days' : 'None'}
            </div>
            <p className="text-xs text-muted-foreground">
              Next invoice due
            </p>
